...
app.post('/create', async (req, res) => {
  const { id_tipo_documento, id_cliente, nombre_cliente, direccion_cliente, celular_cliente, password_cliente } = req.body; // Extrae datos del cuerpo de la solicitud

  try {
    // Ejecuta una consulta para insertar un nuevo cliente en la base de datos
    await sequelize.query(
      `INSERT INTO clientes (id_tipo_documento, id_cliente, nombre_cliente, direccion_cliente, celular_cliente, password_cliente) VALUES (?, ?, ?, ?, ?, ?)`,
      {
        replacements: [id_tipo_documento, id_cliente, nombre_cliente, direccion_cliente, celular_cliente, password_cliente], // Reemplaza los placeholders con los valores reales
